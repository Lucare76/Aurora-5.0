...
test('movimento -> saldo: una spesa passa dalla UI, aggiorna il saldo e viene riletta', async ({ page }) => {
  ...
  await dialog.getByPlaceholder('0,00').fill('24,50')
  await dialog.getByPlaceholder('ES. SPESA SUPERMERCATO').fill('SPESA TEST E2E')
  await dialog.locator('input[type="date"]').fill('2026-09-25')
  await dialog.locator('select').nth(0).selectOption(ACCOUNT_MAIN)
  await dialog.locator('select').nth(1).selectOption(CATEGORY_GROCERIES)
  await dialog.getByRole('button', { name: 'Salva movimento' }).click()
...
test('giroconto: la UI invia sorgente/destinazione e ricarica entrambi i saldi senza cambiare il totale', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Nuovo movimento' }).click()
  const dialog = page.getByRole('dialog', { name: 'Nuovo movimento' })
  await dialog.getByRole('button', { name: 'Trasferimento' }).click()
  await dialog.getByPlaceholder('0,00').fill('100')
  await dialog.getByPlaceholder('ES. SPESA SUPERMERCATO').fill('GIROCONTO TEST E2E')
  await dialog.locator('input[type="date"]').fill('2026-09-25')
  await dialog.locator('select').nth(0).selectOption(ACCOUNT_MAIN)
  await dialog.locator('select').nth(1).selectOption(ACCOUNT_SAVINGS)
  await dialog.getByRole('button', { name: 'Salva movimento' }).click()
