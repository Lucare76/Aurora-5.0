-- Migration 00014: aggiunge pre-check UUID conflict nell'RPC di restore.
--
-- Problema: se il backup proviene dallo stesso progetto Supabase ma da un utente
-- diverso (o lo stesso utente con un nuovo auth UID), gli UUID degli account/categorie
-- nel backup possono collidere con quelli gia' presenti nel DB, producendo
-- "duplicate key value violates unique constraint 'accounts_pkey'".
--
-- Soluzione: controlla i conflitti UUID PRIMA di inserire e solleva UUID_CONFLICT.
...
begin
  ...
  -- UUID conflict check: rileva collisioni globali prima di inserire.
  -- Accade quando il backup proviene dallo stesso DB ma da un utente diverso
  -- (o lo stesso utente con nuovo auth UID) e i dati originali sono ancora presenti.
  if exists (
    select 1 from public.accounts a
    where a.id in (
      select (elem->>'id')::uuid
      from jsonb_array_elements(coalesce(p_backup #> '{data,accounts}', '[]'::jsonb)) elem
      where (elem->>'id') is not null
    )
  ) then
    raise exception 'UUID_CONFLICT';
  end if;
  ...
  if exists (
    ...
    where c.id in (
      select (elem->>'id')::uuid
      from jsonb_array_elements(coalesce(p_backup #> '{data,categories}', '[]'::jsonb)) elem
      where (elem->>'id') is not null
    )
    and c.user_id <> v_uid
  ) then
  ...
  if exists (
    ...
    where t.id in (
      select (elem->>'id')::uuid
      from jsonb_array_elements(coalesce(p_backup #> '{data,transactions}', '[]'::jsonb)) elem
      where (elem->>'id') is not null
    )
  ) then
    raise exception 'UUID_CONFLICT';
