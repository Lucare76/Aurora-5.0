-- Migration 00013: aggiunge 'extensions' al search_path dell'RPC di restore.
--
-- Problema: pgcrypto (funzione digest) e' installato nello schema 'extensions'
-- in Supabase. L'RPC aveva search_path = public, pg_temp e non trovava digest(),
-- producendo "function digest(text, unknown) does not exist".
--
-- Soluzione: ricrea l'RPC con search_path = public, extensions, pg_temp.
