-- Migration 00026: blocca l'IDOR su create_default_categories.
--
-- Problema: create_default_categories(p_user_id uuid) e' security definer,
-- non verifica auth.uid() = p_user_id e non ha mai avuto un revoke/grant
-- esplicito (a differenza di create_transaction_atomic, delete_user_account,
-- create_recurring_transaction, restore_aurora_backup_v1_empty_account).
-- Mantiene quindi il privilegio EXECUTE di default assegnato a PUBLIC
-- (anon + authenticated): un chiamante con la anon key pubblica puo'
-- inserire categorie di default nell'account di un altro utente esistente
-- passando un p_user_id arbitrario (audit di sola lettura 2026-07-29).
--
-- Soluzione: stesso pattern gia' in uso in delete_user_account (00007) --
-- verifica auth.uid() is null / auth.uid() != p_user_id -- piu' search_path
-- esplicito, e revoke/grant esplicito come create_transaction_atomic (00002/00003).
-- Corpo degli INSERT invariato rispetto a 00004_default_categories_with_subcategories.sql.
...
revoke all on function public.create_default_categories(uuid)
from public, anon, authenticated;

grant execute on function public.create_default_categories(uuid)
to authenticated;
