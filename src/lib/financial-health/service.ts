...
export async function buildFinancialHealthPayload(
  ...
  const [
    ...
    supabase.from('transactions').select('id,user_id,account_id,category_id,type,amount,description,notes,date,transfer_peer_id,recurring_id,receipt_url,receipt_data,is_neutral,created_at,updated_at').eq('user_id', userId).gte('date', twelveMonthsAgo).lte('date', in90).order('date', { ascending: true }).limit(10000),
