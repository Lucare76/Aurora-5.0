...
export async function POST(request: Request) {
  ...
  const parsed = affordabilityInputSchema.safeParse(body)
  if (!parsed.success) {
    return json({ error: 'VALIDATION_ERROR', details: parsed.error.issues }, 422)
  }

  const input = parsed.data as AffordabilityInput
  ...
  try {
    ...
    console.error('[aurora-affordability] calculate', err instanceof Error ? err.message : err)
    return json({ error: 'CALCULATION_FAILED', message: 'Non è stato possibile completare la valutazione. Nessun dato finanziario è stato modificato.' }, 500)
  }
}
